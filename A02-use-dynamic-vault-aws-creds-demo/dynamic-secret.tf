...
resource "aws_instance" "web" {
  ...
  tags = {
    Name = "Vault-HelloWorld-terraform.workspace"
  }
}

# Create a user using the dynamic AWS credentials generated
resource "aws_iam_user" "congrats" {
  name = "congrats-it-works"
}

# Create an s3 bucket
resource "aws_s3_bucket" "example" {
  bucket = "my-tf-vault-test-bucket-kemgou"
...
resource "aws_dynamodb_table" "example" {
  name         = "my-tf-vault-test-bucket-kemgou" 
